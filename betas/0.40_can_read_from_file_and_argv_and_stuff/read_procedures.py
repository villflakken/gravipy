...
class readProcedures(readSifters, readTools):
    ...
    def __init__(self):
        ...
        """                   # Allows for 1 file to missing in a sequence,
    ...
    def read_posvel(self):
        ...
        for i in N.arange(0, iterLen):
            ...
            try:
                with open(filepath, 'rb') as openfile:
                    readtext = "\t Accessing file:" \
                          + "    indra%d/snapdir%d/file.%d\t (%s) ..." \
                             % (self.indraN, self.subfolder, i, self.what)
        ...
        # """
        # Converts input list into an array.
        # """
        ...
        # Manual method
        # IDsA = N.zeros( ( iterLen, maxN    ) , dtype=N.int64   )
        # posA = N.zeros( ( iterLen, maxN, 3 ) , dtype=N.float32 )
        # velA = N.zeros( ( iterLen, maxN, 3 ) , dtype=N.float32 )

        # for i in N.arange(0, iterLen):
        #     " Put them into the right places, independent of sorting "
            
        #     itertext = " Assigning (unsorted) values: {0:>3d} / {1} ..."\
        #                 .format(i, (iterLen-1))
        #     " Less spam in the terminal "
        #     self.itertextPrinter(itertext, i, iterLen, 10)

        #     IDsA[ i, :NpartA[i] ]    = IDsL[i][:]
        #     posA[ i, :NpartA[i], : ] = posL[i][:,:]
        #     velA[ i, :NpartA[i], : ] = velL[i][:,:]
        #     continue
